...
@auth_bp.route('/auth/refresh_token', methods=["GET"])
@jwt_required(refresh=True)
def get_token():
    return AuthController.refresh(request)

@auth_bp.route('/auth/send_confirm_email_token', methods=["GET"])
def send_token():
    return AuthController.send_confirm_email(request)

@auth_bp.route('/auth/confirm_email', methods=["POST"])
def confirm_email():
    return AuthController.confirm_email(request)

@auth_bp.route('/auth/forgot_password_token', methods=["GET"])
