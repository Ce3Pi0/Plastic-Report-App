...
class MailHandler:
    ...
    def send_link(self, message: str, recipient: str, link) -> None:
        ...
        msg = Message(message, sender=get_env["MAIL_USERNAME"], recipients=[recipient])
        msg.body = f"Your link is {link}"

        self.__mail.send(msg)

    def __validate_mail_config(self) -> None:
        if not get_env["MAIL_SERVER"] or not get_env["MAIL_USERNAME"] or not get_env["MAIL_PASSWORD"] or not get_env["MY_MAIL"]:
